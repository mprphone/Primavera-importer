...
import { readEncryptedJson } from '../lib/crypto-store.mjs'

const here = dirname(fileURLToPath(import.meta.url))
const script = join(here, '..', 'efatura', 'fetch_efatura.py')

function pythonCommand() {
  const configured = process.env.PRIMAVERA_PYTHON
  if (configured) return { command: configured, prefix: /(^|[\\/])py(?:\.exe)?$/i.test(configured) ? ['-3'] : [] }
...
export async function fetchEfaturaInvoices({ runtimeDirectory, companyCode, dateFrom, dateTo }) {
  const credentials = await readEncryptedJson(
    join(runtimeDirectory, 'credentials', `${companyCode}.json.enc`),
